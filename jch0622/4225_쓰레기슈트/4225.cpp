...
public:
    ...
    double lineToPointDistance(const Point2 & p1, const Point2 & p2) const
    {
        ...
        return p1.dist(*this) * sin(angle * PI / 180);
    }
};
...
public:
    ...
    int findConvexPoint(const int & index) const
    {
        ...
        std::vector<double> angleVec = {0.0, 0.0, 0.0};
        std::vector<int> idxVec = {-1, indexNext, -1};
        ...
        for (int i = 0; i < this->numberOfSide; i++)
        {
            ...
            if (angleVec[ccwValue] < angle)
            {
                angleVec[ccwValue] = angle;
                idxVec[ccwValue] = i;
            }
        }
        if (angleVec[PARALLEL] + EPSILON >= 180 || angleVec[CW] + angleVec[CCW] > 180)
            return -1;
        else
        {
            if (idxVec[CW] >= 0)
